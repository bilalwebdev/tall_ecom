<div class="relative flex flex-col justify-between h-screen shadow-xl dark:shadow-slate-500" x-data="{
    subtotal: @entangle('total'),
    modalOpen: false,
    ...
    <div
        class="flex items-center justify-between p-4 bg-gray-800 shadow-md dark:shadow-none dark:border-b dark:border-b-gray-400 ">
        ...
        <button type="button" x-on:click="cart=false" aria-controls="drawer-right-example"
    ...
    @if ($couponApplied)
        <x-base.small-alert wire:click="clearCoupon">{{ $coupon->code }} {{ __('cart.applied_successfully') }}
        </x-base.small-alert>
    @endif

    <div x-show="subtotal<=0"
        class="px-4 py-2 mx-4 mt-4 mb-auto text-center border-2 border-dashed dark:border-gray-400">
                    ...
                    <li class="flex py-4" wire:key="{{ $id }}-{{ $product['slug'] }}">
                        ...
                        <div class="flex flex-col flex-1 ml-4">
                            ...
                            <div class="flex items-center justify-between flex-1 text-sm">
                                ...
                                <div class="flex">
                                    <button type="button" class="font-medium text-rose-500 hover:text-rose-400"
    ...
    <div x-show="subtotal>0" class="p-4 border-t border-gray-300 dark:border-gray-400 sm:px-6">
        <div class="flex justify-between text-base font-medium text-gray-900 dark:text-white">
            ...
            @if ($couponApplied)
                <p><span class="mr-1 text-sm text-red-400 line-through">({{ format_money($total) }})</span>
                    {{ format_money($total - $discount) }} </p>
            @else
            ...
            <div class="flex items-center justify-center">
                <div>
                    <div class="relative mt-1 rounded-md shadow-sm">
                        <input type="text" {{ $couponApplied ? 'disabled' : '' }}
                            placeholder="{{ __('cart.promocode') }}" name="promo" wire:model.lazy="promo"
                            id="promo"
                        ...
                        @enderror"
                            aria-invalid="false" aria-describedby="promo-error" wire:keydown.enter="applyPromo">
                        @error('promo')
                    ...
                    @error('promo')
                        <p class="mt-2 text-sm text-red-600" id="promo-error">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <div x-show="modalOpen" x-transition:enter="transition ease-in duration-200"
        ...
        class="absolute right-[15%] top-[30%] z-50 p-3 bg-white rounded-lg max-w-[70%]" @click.away="closeConfirm()">
