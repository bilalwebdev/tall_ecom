<div class="max-w-2xl px-4 pt-12 pb-24 mx-auto sm:px-6 lg:max-w-7xl lg:px-8">
    <h2
        class="block mb-10 text-3xl font-bold leading-8 tracking-tight text-center text-gray-900 dark:text-white sm:text-4xl">
        {{ __('cart.checkout') }}</h2>
    ...
    @if ($total > 0)
        <form class="lg:grid lg:grid-cols-2 lg:gap-x-12 xl:gap-x-16" @submit.prevent>
            <div>
                <div>
                    ...
                    <div class="mt-4">
                        ...
                        @else
                            <x-form.input name="email" type="email" label="{{ __('form.email') }}" autofocus
                                autocomplete="email" value="{{ auth()->check() ? auth()->user()->email : '' }}" />
                        @endauth
                    </div>
                </div>
                ...
                <div class="pt-8 mt-8 border-t border-gray-200 dark:border-gray-400">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('checkout.shipping_info') }}
                        </h2>
                        @auth
                            <div class="flex items-center">
                                <button type="button"
                                    class="<?= $add_new_address ? 'bg-rose-600' : 'bg-gray-200' ?> relative inline-flex flex-shrink-0 h-5 transition-colors duration-200 ease-in-out border-2 border-transparent rounded-full cursor-pointer w-10 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2"
                                    role="switch" aria-checked="{{ $add_new_address ? 'true' : 'false' }}"
                                    aria-labelledby="add-new-address-label"
                                    @click="$wire.set('add_new_address', {{ !$add_new_address }})">
                                    <span aria-hidden="true"
                                        class="<?= $add_new_address ? 'translate-x-5' : 'translate-x-0' ?> inline-block w-4 h-4 transition duration-200 ease-in-out transform translate-x-0 bg-white rounded-full shadow pointer-events-none ring-0"></span>
                                </button>
                    ...
                    @if (isset($addresses) && !$add_new_address)
                        <div class="grid grid-cols-1 gap-3 mt-3 md:gap-4 md:grid-cols-3" x-data="{ address: @entangle('selected_address').defer }">
                            @foreach ($addresses as $address)
                                <label x-bind:class="{ 'ring-2 ring-rose-500': address === '{{ $address->id }}' }"
                                    class="relative flex p-4 bg-white rounded-lg shadow-sm cursor-pointer dark:bg-gray-300 focus:outline-none">
                                    <input type="radio" name="delivery-method" x-model="address"
                                        value="{{ $address->id }}" class="sr-only"
                                        aria-labelledby="delivery-method-0-label"
                                        aria-describedby="description-{{ $loop->index }}"
                                        @click="$wire.set('selected_address', address!=='{{ $address->id }}' ? '{{ $address->id }}': null)">
                                    <span class="flex flex-1">
                                        <span class="flex flex-col">
                                            <span id="delivery-method-0-label"
                                                class="block text-sm font-medium text-gray-900 capitalize">Address</span>
                                            <span id="description-0" class="mt-1 text-sm text-gray-500">
                                                {{ "{$address->address_line_1}, {$address->address_line_2}" }}</span>
                                            <span id="description-1" class="mt-1 text-sm text-gray-500">
                                                {{ "{$address->city}, {$address->state}, {$address->zip_code}" }}</span>
                                            <span id="description-2" class="mt-1 text-sm text-gray-500">
                                                {{ $address->country }}</span>
                                        </span>
                                    </span>
                                    <template x-if="address === '{{ $address->id }}'">
                                        <svg class="w-5 h-5 text-rose-600" xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <span
                                        x-bind:class="{ 'border-2 border-rose-500': address === '{{ $address->id }}' }"
                                        class="absolute border rounded-lg pointer-events-none -inset-px"
                            ...
                            @guest()
                                <div class="flex items-center my-4">
                                    <input type="checkbox" id="checkbox"
                                        class="w-5 h-5 text-rose-600 form-checkbox dark:text-rose-400"
                                        wire:model.lazy="registration">
                                    <label for="checkbox"
                                        class="block ml-2 text-sm text-gray-900 dark:text-gray-200">{{ __('checkout.keep_info') }}</label>
                                </div>
                            @endguest
                        </div>
                    @endif
                ...
                <div class="pt-10 mt-10 border-t border-gray-200 dark:border-gray-400" x-data="{ delivery: @entangle('delivery_method').defer }">
                    <fieldset>
                        <legend class="text-lg font-medium text-gray-900 dark:text-white">
                            {{ __('checkout.delivery_method') }}</legend>

                        <div class="grid grid-cols-1 mt-4 gap-y-6 sm:grid-cols-3 sm:gap-x-4">
                            @foreach ($delivery_charge as $key => $value)
                                <label x-bind:class="{ 'ring-2 ring-rose-500': delivery === '{{ $key }}' }"
                                    class="relative flex p-4 bg-white rounded-lg shadow-sm cursor-pointer dark:bg-gray-300 focus:outline-none">
                                    <input type="radio" name="delivery-method" x-model="delivery"
                                        value="{{ $key }}" class="sr-only"
                                        aria-labelledby="delivery-method-0-label"
                                        aria-describedby="delivery-method-0-description-0 delivery-method-0-description-1"
                                        @click="delivery!=='{{ $key }}'?$wire.updateDeliveryMethod('{{ $key }}'):null">
                                    <span class="flex flex-1">
                                        <span class="flex flex-col">
                                            <span id="delivery-method-0-label"
                                                class="block text-sm font-medium text-gray-900 capitalize">{{ __("checkout.{$key}") }}</span>
                                            <span id="delivery-method-0-description-0"
                                                class="flex items-center mt-1 text-sm text-gray-500">
                                                {{ __('checkout.business_days', ['days' => $value == 5 ? '5-10' : '2-4']) }}</span>
                                            <span id="delivery-method-0-description-1"
                                                class="mt-6 text-sm font-medium text-gray-900">{{ format_money($value) }}</span>
                                        </span>
                                    </span>
                                    ...
                                    <span
                                        x-bind:class="{ 'border-2 border-rose-500': delivery === '{{ $key }}' }"
                                        class="absolute border rounded-lg pointer-events-none -inset-px"
                                        aria-hidden="true"></span>
                                </label>
                            @endforeach
                    ...
                    <div class="grid grid-cols-4 mt-6 gap-y-6 gap-x-4">
                        @if ($payment_method === 'creditcard')
                            <div class="col-span-4">
                                <label for="card-number"
                                    class="block text-sm font-medium text-gray-700 dark:text-white">{{ __('form.card_number') }}</label>
                                <div class="mt-1">
                                    <input type="text" id="card-number" name="card-number"
                                        autocomplete="cc-number"
                                        class="block w-full border-gray-300 rounded-md shadow-sm focus:border-rose-500 focus:ring-rose-500 sm:text-sm">
                                </div>
                            </div>

                            <div class="col-span-4">
                                <label for="name-on-card"
                                    class="block text-sm font-medium text-gray-700 dark:text-white">{{ __('form.name_on_card') }}</label>
                                <div class="mt-1">
                            ...
                            <div class="col-span-3">
                                <label for="expiration-date"
                                    class="block text-sm font-medium text-gray-700 dark:text-white">{{ __('form.expiration') }}</label>
                                <div class="mt-1">
                            ...
                            <div class="col-span-1">
                                <label for="cvc"
                                    class="block text-sm font-medium text-gray-700 dark:text-white">CVC</label>
                                <div class="mt-1">
                        ...
                        @else
                            <div class="flex justify-center col-span-4">
                                <img class="h-32" src="{{ asset('images/cash-on-delivery.webp') }}"
            ...
            <!-- Order summary -->
            <div class="mt-10 lg:mt-0" x-data="{
                subtotal: @entangle('total').defer,
                modalOpen: false,
                ...
                <div class="relative mt-4 border border-gray-200 rounded-lg shadow-sm dark:border-gray-400">
                        ...
                        <ul role="list" class="px-4 divide-y divide-gray-200" wire:key="alpine">
                            @foreach ($content as $id => $product)
                                <li class="flex py-4" wire:key="{{ $id }}-{{ $product['slug'] }}">
                                    <div
                                        class="flex-shrink-0 w-16 h-16 overflow-hidden border border-gray-300 rounded-md dark:border-gray-400">
                                    ...
                                    <div class="flex flex-col flex-1 ml-4">
                                        ...
                                        <div class="flex items-center justify-between flex-1 text-sm">
                                            <div class="flex items-center mt-2">
                                                @php
                                                    $disallowMinus = $product['quantity'] <= 1;
                                                    $disallowPlus = $product['quantity'] >= $product['stock'];
                                                @endphp
                                                <button
                                                    class="<?= $disallowMinus ? 'cursor-not-allowed' : '' ?> text-gray-500 dark:text-gray-300 focus:outline-none focus:text-gray-600 dark:focus:text-gray-100"
                                                    wire:click="updateCartItem({{ $id }}, 'minus')"
                                                    <?= $disallowMinus ? 'disabled' : '' ?>>
                                                ...
                                                <p class="mx-2 text-gray-700 dark:text-gray-100">
                                                    {{ $product['quantity'] }}</p>
                                                <!-- $wire.updateCartItem(id, 'plus') -->
                                                <button
                                                    class="<?= $disallowPlus ? 'cursor-not-allowed' : '' ?> text-gray-500 dark:text-gray-300 focus:outline-none focus:text-gray-600 dark:focus:text-gray-100"
                                                    wire:click="updateCartItem({{ $id }}, 'plus')"
                                                    <?= $disallowPlus ? 'disabled' : '' ?>>
                                                    <svg class="w-5 h-5" fill="none" stroke-linecap="round"
                                                        stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24"
                                                        stroke="currentColor">
                                                        <path
                                            ...
                                            <div class="flex">
                                                <button type="button"
                                                    class="font-medium text-rose-500 hover:text-rose-400"
                                                    @click="isConfirm($wire.removeFromCart, [{{ $id }}])">
                    ...
                    <dl class="px-4 py-6 space-y-6 border-t border-gray-200 dark:border-gray-400 sm:px-6">
                        <div class="flex items-center justify-between">
                            ...
                            <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ format_money($total) }}
                            </dd>
                        </div>
                        @if ($couponApplied)
                            <div class="flex items-center justify-between">
                                <dt class="text-sm dark:text-white">{{ __('checkout.discount') }}
                                    ({{ $coupon->value }}{{ $coupon->type === 'percent' ? '%' : '' }})</dt>
                                <dd class="text-sm font-medium text-gray-900 dark:text-white">
                                    {{ format_money($discount) }}</dd>
                            </div>
                        @endif
                        <div class="flex items-center justify-between">
                            <dt class="text-sm dark:text-white">{{ __('checkout.shipping') }}
                                ({{ __("checkout.{$delivery_method}") }})</dt>
                            <dd class="text-sm font-medium text-gray-900 dark:text-white">
                                {{ format_money($delivery_charge[$delivery_method]) }}</dd>
                        </div>
                        <div class="flex items-center justify-between">
                            <dt class="text-sm dark:text-white">{{ __('checkout.tax') }} ({{ $taxrate }}%)</dt>
                            <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ format_money($taxable) }}
                            </dd>
                        </div>
                        <div
                            class="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-400">
                            <dt class="text-base font-medium dark:text-white">{{ __('cart.total') }}</dt>
                            <dd class="text-base font-medium text-gray-900 dark:text-white">
                                {{ format_money($grand_total) }}</dd>
                        </div>
                    </dl>

                    <div class="px-4 py-6 border-t border-gray-200 dark:border-gray-400 sm:px-6">
                        @if ($couponApplied)
                            <x-base.small-alert wire:click="clearCoupon">{{ $coupon->code }}
                                {{ __('cart.applied_successfully') }}</x-base.small-alert>
                        @endif
                        <div class="flex items-end w-full my-4 justify-items-stretch">

                            <x-form.input name="promo" label="{{ __('cart.apply_promo') }}"
                                placeholder="{{ __('cart.promocode') }}" />

                            <div class="flex-shrink-0 mb-1 ml-2">
                                <x-base.loading-button wire:click="applyPromo" wire:loading.attr="disabled"
                                    loading="true">
                        ...
                        <x-base.loading-button wire:click.prevent="submit" wire:loading.attr="disabled"
                            loading="true"
                            class="justify-center w-full px-4 py-3 text-base font-medium text-center text-white bg-rose-600 border border-transparent rounded-md shadow-sm hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2 focus:ring-offset-gray-50 !ml-0">
                            <span> {{ __('checkout.confirm_order') }}</span>
                        </x-base.loading-button>
                    </div>
                    <div x-cloak x-show="modalOpen" class="absolute inset-0 z-10 w-full h-full bg-black opacity-25">
                    </div>
                    <div x-cloak x-show="modalOpen" x-transition:enter="transition ease-in duration-200"
                        x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100"
                        x-transition:leave="transition ease-out duration-300"
                        x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90"
                        class="absolute right-[33%] top-[10%] z-50 p-3 bg-white dark:bg-gray-200 rounded-lg max-w-[70%]"
                        ...
                        <div class="mt-4 text-center md:text-right md:flex md:justify-end">
                            <button
                                class="block w-full px-2 py-1 text-sm font-semibold text-red-700 bg-red-200 rounded-lg md:inline-block md:w-auto md:ml-2 md:order-2"
                                @click="execute instanceof Function?(execute(...args), closeConfirm()):$store.toasts.createToast('Something went Wrong, try Again', 'error')">{{ __('common.yes') }}</button>
                            <button
                                class="block w-full px-2 py-1 mt-4 text-sm font-semibold bg-gray-200 rounded-lg md:inline-block md:w-auto md:mt-0 md:order-1"
                                @click="closeConfirm()">{{ __('common.no') }}</button>
                        </div>
                    </div>
                </div>
            </div>
