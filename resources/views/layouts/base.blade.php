...
$watch('darkMode', value => {
    ...
    <div x-data="{ open: false, cart: false }" @keydown.window.escape="open = false" class="bg-white dark:bg-gray-800">
        ...
        <div x-cloak class="relative z-40">
            <div x-show="cart" x-transition:enter="ease-in-out duration-500" x-transition:enter-start="opacity-0"
                x-transition:enter-end="opacity-100" x-transition:leave="ease-in-out duration-500"
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75"></div>
            <div :class="cart ? 'translate-x-0 ease-out' : 'translate-x-full ease-in'"
                class="fixed top-0 right-0 z-40 w-full h-full max-w-xs overflow-hidden transition duration-300 transform bg-white border-l border-gray-300 shadow-xl dark:bg-gray-800 dark:border-gray-600">
                <livewire:cart />
            </div>
        </div>
        <x-base.footer />
        <div x-data class="absolute top-0 z-50 w-64 p-4 overflow-x-hidden inset-x-50">
            <template x-for="(toast, index) in $store.toasts.list" :key="toast.id">
                <div x-show="toast.visible" @click="$store.toasts.destroyToast(index)"
                    ...
                    class="flex items-center p-3 mb-3 text-white bg-gray-900 rounded shadow-lg bg-gradient-to-r"
                    :class="{
